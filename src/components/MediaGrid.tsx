...
export function MediaGrid({
  ...
  const muted = tone === "light" ? "text-cream/70" : "text-ink/60";
  const grid =
    columns === "two"
      ...
      <div className="mx-auto max-w-md text-center">
        <div
          className={`painted-edge border border-dashed px-8 py-10 ${
            tone === "light" ? "border-gold/40 bg-cream/[0.05]" : "border-gold/45 bg-cream/50"
          }`}
        >
          <svg
            viewBox="0 0 48 48"
          ...
          <p className={`mt-4 font-body text-base italic ${muted}`}>{message}</p>
          {!isSupabaseConfigured && !error && (
            <p className={`mt-2 font-body text-xs not-italic ${muted}`}>
      ...
      <div className={`grid gap-4 sm:gap-6 ${grid}`}>
        {items.map((item, index) => (
          ...
          >
            ...
            {item.caption && (
              <span className="absolute inset-x-0 bottom-0 z-10 bg-gradient-to-t from-maroon-950/85 to-transparent px-3 pb-3 pt-8 text-left font-body text-sm text-cream opacity-0 transition-opacity duration-500 group-hover:opacity-100">
                {item.caption}
              </span>
            )}
          </motion.button>
